...
リゾルバーでユーザーグループを参照できるので、それを使って項目ごとの認可制御ができます。
以下の例では、「管理者」「営業」「一般ユーザー」のCognitoユーザーグループがある場合に、
タスク情報の登録日と更新日については「管理者」「営業」のみが参照可能できるように制御しています。
...
例えば「一般ユーザーは自分が登録した情報のみ参照でき、管理者はすべて参照できる」のような要件を実現する場合です。
...
```jsx{2,15}:title=ユーザー名とユーザーグループで認可制御する例
## Adminか判定
#foreach($group in $context.identity.claims.get("cognito:groups"))
  #if($group == "admin")
    #set($isAdmin = true)
...
これで`taumon`の場合は２件、`inoue`の場合は1件、管理者の場合は3件参照できるようになります。
リゾルバー内で`$util.unauthorized()`を使えば認可エラーを返せるので、頑張ってリゾルバーを書けば複雑な要件も実現できるでしょう。
...
### AWSコンソールの場合
手っ取り早く試すなら、AppSyncの画面のQueryに認証機能がついているので、そちらで試しましょう。
...
* するとログインモーダルが出てきます。接続先のCognitのクライアントIDとともにユーザー情報を入力すればOKです。

![](./how-to-client-login-2.png)
<br/>

* あとは通常のGraphQLのIDEと同じようにQueryやMutaitonを発行すればレスポンスが確認できます。
