...
ビルド時間の内訳を見たところ、`Generating image thumbnails`に10分以上かかっておりビルド時間の大半を締めていました。
...
設定方法は[Netlifyの記事](https://www.netlify.com/blog/2020/02/25/gatsby-build-speed-improvements-with-parallel-image-processing/)にキャプチャ付きで詳しく載っているのでそちらをご覧いただければ簡単に設定できると思います。GCPの環境をCLIで構築してNetlifyに環境変数を設定すればすぐにgatsby-parallel-runnerが使えます。


## 2. 必要な画像だけクエリで取得する

以下のように全ファイルを取得するようなクエリは、全ての画像を取得してしまうのでビルド時間が膨大になってしまいます。

```graphql{2}:title=だめな例
query {
  allFile {
    edges {
      node {
...
クエリで取得する画像は他画像とフォルダを分けて、クエリのフィルタ条件を指定しましょう。
そうすることで不必要に画像を生成せずに済み、ビルド時間を短縮できます。
...
3つのサムネイル画像だけ取得するクエリを定義したいけれど、クエリ中に３つの画像のパスをフィルタ条件としてハードコードしたくないので、一旦、全てのサムネイルをクエリで取得して、JS(コンポーネント)中で3つにフィルタするようなケースがあります。

StaticQueryなどは、その名の通りStaticなのでJS中の変数をフィルタ条件として使うことができませんが、
ページテンプレートに指定しているコンポーネントのクエリではContextがフィルタ条件として使えます。
...
以下のように同じ画像でも、記事埋め込み用、OGP画像用などに違う縦横サイズ、クオリティを指定していると、画像生成枚数が倍増しビルド時間も増えます。

```graphql{1,7,16,22}:title=生成画像枚数が倍になってしまうクエリの指定例
...
```graphql{1,7}:title=生成画像枚数を減らすためのクエリの指定例
...
画像のサイズが大きい場合、ビルド時間は指数関数的に増加します。
そのため、表示幅に併せて画像の縦横比を縮小しましょう。
例えば記事中で最大800pxで表示するような画像の場合、Retina対応も考慮すると最大幅は800px * 2 = 1600pxで十分です。
この場合、ビルド前に1600pxより幅が大きい画像は全て1600pxに縮小しましょう。
...
```javascript:title=scripts/optimize-images
...
Netlifyのビルドでタイムアウトを解決したい場合には根本的な解決策にはならないので、気をつけてください。キャッシュでビルド時間がタイムアウト時間以内に収まっていたとしても、変更範囲の大きい機能回収でキャッシュが効かない場合や、なんらかの不具合でキャッシュなしでクリーンビルドが必要になった場合、結局タイムアウトが発生してしまいます。
この場合は先にあげた1～5を実施して、根本的なビルド時間の短縮を図ってください。
...
本線からは少し脱線しますが、どうしてもNetlifyでタイムアウト問題が克服できない or ビルド時間が安定しない時はCircle CIを使ってビルドすると良いです。Netlify側でコケても、CircleCIでビルド検証ができるようになります。
