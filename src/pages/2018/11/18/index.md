---
title: ファイル名を指定して実行（windws + r）をランチャーとして使う
...
いろいろ便利なランチャーありますが、Windows PCだと僕はファイル名を指定して実行（`windows + r`）をランチャー代わりにしています。
この方法は、どんなWindows PCでもすぐに使えるというのが最大の利点です。
PCリプレースしたとき、ネットにつながらない開発現場に行ったときなどでも適用できます。
ただこのやり方、会社の同僚などで使ってる人に遭遇したことはありません。マイナーなんですかね:cry:
つい最近Surface Pro 6を買ったついでに、今回は`windows + r`ランチャーの設定方法・使い方をメモします。
...
今は4年前くらいのMac Book Air(リンゴが光る世代のやつ)を使ってて、ちょうど[Airの新型発表されたし](https://www.apple.com/jp/macbook-air/)買おっかなとか思ったんですけど、会社のリモートワーク用端末としてはWindowsが良さそうだし、IEとEdgeの検証環境ほしいってことで[Surface Pro 6](https://www.microsoft.com/ja-jp/p/surface-pro-6/8zcnc665slq5?activetab=pivot%3aoverviewtab)買いました。基本リモート接続で使うので最低スペックのやつです。
一週間使ってみました。サブPCとしてはとてもいい感じです！

### メリット

* 軽い(784g 自分のMBA(1350g)に比べるとかなり軽い)
* 電源コードがMacのマグセーフ形式で脱着が容易
* 顔認証で簡単ログイン
* タブレットとしても使用可能（家で漫画やYouTube見るのに最適）

### デメリット

* 別売りキーボードは２万円...東プレのREALFORCE並みにお高い
...
パスの通ったランチャー専用フォルダをつくって、
そこにフォルダショートカット、アプリショートカット、バッチなどを置きます。
そうすると`windows + r`で呼び出せるという仕組みです。

まずはランチャー専用のフォルダを作成します。自分は`C:\shrotcut`としました。

![ランチャー用フォルダ作成](./setting-1.png)

「環境変数名の編集」でパスを通します。

![ランチャー用フォルダ作成](./setting-3.png)
...
試しに「ランチャー用フォルダ」のショートカットを配置してみましょう。ショートカット名は`shortcut`にします。

![ランチャー用フォルダ作成](./setting-2.png)

`windows + r` → `shortcut`と入力 →　`Enter`キー　でフォルダが開けば設定成功です。
...
```txt
"C:\Users\xxx\AppData\Local\Programs\Microsoft vS code\Code.exe" C:\Users\inoue\workspace\memo
```
...
作成したショートカットをランチャー用フォルダに格納すればVSCodeやChromeなどが呼び出せるようになります。

### コントロールパネルの設定を起動する

コントロールパネルを開いてドラック＆ドロップでショートカットが作れます。
作成したショートカットを好きな名前にリネームしてランチャー用フォルダに格納すればサウンド設定などが呼び出せるようになります。
...
使いそうなものだけ下記に列挙します。
※電卓、メモ帳、ペイントあたりは使わないかなぁと思い列挙していません。
...
例えば下記のようなバッチファイルを作成してランチャー用フォルダに配置します。ここではファイル名を`blue.bat`とします。
...
ms-setttings:URLの一覧はsystemsettings.dllに記載されています。Stringsコマンドで確認できます。
...
バッチファイルやパワーシェルファイルをランチャー用フォルダに配置すれなんでもできます。
例えばshutdownコマンドでシャットダウンや再起動ができます。下記のようなバッチを作ってランチャー用フォルダに配置しましょう。
...
Windows10からは`ms-settins`も加わって用途が拡大されているのでますます便利になりました。
