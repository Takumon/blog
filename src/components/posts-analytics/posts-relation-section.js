...
const useThumbnails = () => {
  ...
  const result = useMemo(
    () => {
      return data.images.edges.map(edge => {
...
const PostRelationSection = ({ posts }) => {
  ...
  const fullscreenButton = isFull ? (
    <button onClick={goNotFull} style={{ cursor: 'pointer' }}>
      <FontAwesomeIcon color="#333" size="sm" className={styles.icon} icon={faCompress} />
      Back
    </button>
  ) : (
    <button onClick={goFull} style={{ cursor: 'pointer' }}>
      <FontAwesomeIcon color="#333" size="sm" className={styles.icon} icon={faArrowsAlt} />
      Go Fullscreen
    </button>
  )
  ...
  const ContentLoading = <div className={styles.loading}>Now loading...</div>

  const Content = (
    <>
      <div className={styles.fullscreen}>{fullscreenButton}</div>
      <CytoscapeComponent
        zoom={zoomLevel}
        pan={{
          x: 100,
  ...
  return (
    <>
      <h2 className={styles.title}>Posts Relation</h2>
